...
'title' => 'Manage Jenis Transaksi BMM',
'contentTitle' => 'Manage Jenis Transaksi BMM',
])
...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <a href="{{ route('jenistransaksibmm.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus-square"></i> Tambah Data</a>
  </div>
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-striped" id="dataTable1">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $no++ }}</td>
            <td>{{ $jenistransaksi->nm_jenis_transaksi }}</td>
            <td>{{ $jenistransaksi->tipe_transaksi }}</td>
            <td>
              <div class="row ml-3">
                <a href="{{ route('jenistransaksibmm.edit',$jenistransaksi->id) }}" class="btn btn-primary btn-sm">Ubah</a>
                <form method="POST" action="{{ route('jenistransaksibmm.destroy',$jenistransaksi->id) }}">
                  @csrf
                  @method('DELETE')
