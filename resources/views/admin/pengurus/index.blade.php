...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <a href="{{ route('admin.pengurus.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus-square"></i> Tambah Data</a>
  </div>
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-striped" id="dataTable1">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $no++ }}</td>
            <td>{{ $pgs->nm_pengurus }}</td>
            <td>{{ $pgs->kd_pengurus }}</td>
            <td>{{ $pgs->no_hp }}</td>
            <td>{{ $pgs->nm_jabatan }}</td>

            <td>
              <div class="row ml-3">
                <a href="{{ route('admin.pengurus.edit',$pgs->id) }}" class="btn btn-primary btn-sm">Ubah</a>
                <form method="POST" action="{{ route('admin.pengurus.destroy',$pgs->id) }}">
                  @csrf
                  @method('DELETE')
