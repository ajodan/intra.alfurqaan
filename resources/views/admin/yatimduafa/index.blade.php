...
'title' => 'Manage Yatim dan Duafa',
'contentTitle' => 'Manage Yatim dan Duafa',
])
...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <a href="{{ route('admin.yatimduafa.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus-square"></i> Tambah Data</a>
  </div>
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-striped" id="dataTable1">
        ...
        <tbody>
          ...
          $no=1;
          @endphp
          @foreach($yatimduafa as $yatim)
          <tr>
            <td>{{ $no++ }}</td>
            <td>{{ $yatim->nm_lengkap }}</td>
            <td>{{ $yatim->tgl_lahir }}</td>
            <td>{{ $yatim->jk }}</td>
            <td>{{ $yatim->hp }}</td>
            <td>
              <div class="row ml-3">
                <a href="{{ route('admin.yatimduafa.edit',$yatim->id) }}" class="btn btn-primary btn-sm">Edit</a>
                <form method="POST" action="{{ route('admin.yatimduafa.destroy',$yatim->id) }}">
                  @csrf
                  @method('DELETE')
